...
百万 PDF 本地 MinerU 流水线（生产级健壮版本）。

================================================================================
总体架构（三阶段流水线）：
  阶段一【扫描/喂料】feed_tasks()
    └─ 遍历 PDF 目录，查断点续传状态，跳过已完成文件，将待处理任务放入 pending_queue

  阶段二【解析】run_parser_supervisor() → local_parser_worker() (多 GPU 子进程)
    ├─ MinerU 本地库解析 PDF → Markdown
    ├─ MarkdownStructureAdapter.build_chunks() → Markdown → Chunk 列表
    └─ 结果 pickle 序列化到 spool 目录，放入 parsed_queue

  阶段三【向量化+入库】index_worker() (多个 asyncio 协程)
    ├─ 从 spool 反序列化 chunk 列表
    ├─ build_text_fragments / build_sentence_fragments → 超长文本安全切分
    ├─ async_embed_texts → 向量化（自适应 batch_size）
    ├─ 写入 7 个 ES 索引（title/abstract/keyword/fulltext/paragraph/sentence/reference）
    └─ 记录成功到 checkpoint 文件

关键设计点：
  1. 断点续传：pipeline_status.jsonl 记录每个 lngid 的成功/失败状态，重启后自动跳过已完成
  2. 进程级 GPU 隔离：每个 Worker 子进程绑定独立 GPU，通过 Queue 与主进程通信
  3. 超长文本保护：embedding 前对 chunk 做 fragment 拆分（通常不触发，仅兜底）
  4. 自适应降级：Embedding API 遇 413/5xx 自动减半 batch_size 重试，ES _bulk 同理
  5. Worker 健康监控：超时检测、崩溃计数、自动重启/熔断
  6. 资源清理：spool 文件即用即删，临时目录及时清理
================================================================================
"""
...
# ── GPU 监控（可选）────────────────────────────────────────────
# pynvml 用于在分配任务前检查 GPU 显存占用，超过 95% 则跳过该 slot
try:
    import pynvml
...
def analyze_pdf_locally(
    ...
    流程：
      ...
      2. pipeline_doc_analyze: 版面分析 + OCR + 公式/表格识别
      3. result_to_middle_json: 模型输出 → 中间 JSON 结构
      4. union_make():         中间 JSON → Markdown 文本
      5. MarkdownStructureAdapter.build_chunks(): Markdown → Chunk 列表
    ...
    # MinerU 核心管线：版面分析 → 中间 JSON → Markdown
...
async def run_pipeline(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    print(f"GPU配置: {args.gpus}  |  每卡Worker数: {args.workers_per_gpu}  |  显存: {args.vram}GB")
